...
export default function CareerStoriesSectionsContent() {
  ...
  // Initialize formData for 5 sections
  const [formData, setFormData] = useState<FeatureSection[]>(
    Array.from({ length: sectionsCount }, () => ({
      title: "",
      subtitle: "",
      categories: "",
      tags: "",
  ...
  // Drag state for each section (same length as formData)
  const [dragStates, setDragStates] = useState<boolean[]>(
    Array.from({ length: sectionsCount }, () => false)
  )

  // Refs for file inputs (same length as sections)
  const fileInputRefs = useRef<Array<HTMLInputElement | null>>(
    Array.from({ length: sectionsCount }, () => null)
  )
  ...
  const handleFileChange = useCallback((index: number, file: File) => {
    setFormData(prev =>
      prev.map((section, i) => (i === index ? { ...section, image: file } : section))
    )
  }, [])
  ...
  const handleInputChange = (
    index: number,
    field: keyof Omit<FeatureSection, "image">,
    value: string
  ) => {
    setFormData(prev =>
                  ...
                  >
                    ...
                    {section.image && (
                      <p className="mt-2 text-sm text-gray-700">
                        Selected file: {section.image.name}
                      </p>
                    )}
                  </div>
                </div>
