...
import { deleteTag, fetchAllTags } from "@/services/tag.service";
import AddOrUpdateTagModal from "./_components/add-or-update-tag-modal";
...
export default function TagManagementPage() {
  ...
  // permissions
  const [tagCreatePerm, setTagCreatePerm] = useState(false);
  const [tagEditPerm, setTagEditPerm] = useState(false);
  const [tagDeletePerm, setTagDeletePerm] = useState(false);
  ...
  const fetchTags = async () => {
    ...
    try {
      ...
      if (res?.status && Array.isArray(res.data)) {
        const transformed: Tag[] = res.data.map((item) => ({
          _id: item._id,
          name: item.name,
          status: item.status,
        ...
        setTags(transformed);
        const total = transformed.length; // backend not paginated yet
  ...
  useEffect(() => {
    ...
    const checkPermissions = async () => {
      setTagCreatePerm(await hasPermission("tag:create"));
      setTagEditPerm(await hasPermission("tag:update"));
      setTagDeletePerm(await hasPermission("tag:delete"));
    };
    checkPermissions();
  }, [page]);
  ...
  const toggleAll = () => {
    if (selectAll) {
      ...
      setSelectedRows(tags.map((v) => v._id));
    }
  ...
  }, [tags, globalSearchQuery]);

  // local pseudo-pagination
  const startIndex = (page - 1) * limit;
  const paginatedTags = filteredTags.slice(startIndex, startIndex + limit);
                  ...
                  <tr key={tag._id} className="border-b border-gray-200 hover:bg-gray-50">
                    <td className="p-4">
                      <Checkbox
                        ...
                        onCheckedChange={() => toggleRow(tag._id)}
                    ...
                    <td className="p-4">
                      <Badge
                        className={cn(
                          ...
                          tag.status ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"
                        )}
                      >
