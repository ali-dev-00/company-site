...
const NavLink = ({ href, children }: { href: string; children: React.ReactNode }) => (
  <Link
    href={href}
    className="relative text-sm font-medium text-white transition-colors duration-300 hover:text-red-500
...
const MobileNavLink = ({ href, children }: { href: string; children: React.ReactNode }) => (
  <Link
    href={href}
    className="relative flex w-full items-center py-2 text-lg text-white transition-colors duration-300 hover:text-red-500
               after:content-[''] after:absolute after:bottom-0 after:left-1/2 after:-translate-x-1/2
               after:w-0 after:h-[2px] after:bg-[#FF2424] after:transition-all after:duration-300
               hover:after:w-full hover:after:left-0 hover:after:translate-x-0"
  >
      ...
      <div className="max-w-[1366px] mx-auto flex h-20 w-full items-center  px-4 md:px-8 lg:px-16 border-b border-black">
        ...
        >
          <div className="flex items-center gap-2 mb-6 w-full justify-between">
            ...
            <Button variant="ghost" size="icon" className="h-8 w-8 border border-gray-200 bg-gray-100 hover:oapcity-90 cursor-pointer" onClick={toggleMobileMenu}>
              <X className="h-5 w-5" />
