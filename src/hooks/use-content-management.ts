...
export const useContentManagement = <T extends ContentSection = ContentSection>(
  ...
  const fetchContent = useCallback(async () => {
    try {
      ...
      // Try to get existing content - this will use proper authentication
      const existingContent = await safeGetContent(sectionName);
      
      if (existingContent) {
        // Content exists, try to parse it
        try {
          const parsedContent = JSON.parse(existingContent.sectionContent);
          setContent(parsedContent as T);
          setContentExists(true);
        } catch (parseError) {
          console.warn(`Invalid JSON in section ${sectionName}, using default content`, parseError);
          const initialContent = (defaultContent || {}) as T;
          setContent(initialContent);
          setContentExists(false);
        }
      } else {
        // Content doesn't exist, initialize with default
        const initialContent = (defaultContent || {}) as T;
        setContent(initialContent);
        setContentExists(false);
      }
    } catch (err) {
      console.warn(`Error fetching content for section ${sectionName}:`, err);
